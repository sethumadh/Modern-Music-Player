...
export default function TrendSongs({ song, index, data, isPlaying }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center lg:flex-row lg:justify-around w-1/3 ">
        <div className="flex items-center">
          <h3 className="hidden lg:block p-4 text-gray-400 text-left">
            {song.title}
          </h3>
        </div>
        <div className="flex items-center">
          <h3 className="hidden lg:block p-4 text-gray-400">5:00</h3>
        </div>
        <div className="flex items-center justify-center cursor-pointer">
          <HiOutlineHeart className="w-8 h-8 " color="white" />
        </div>
      </div>
    </div>
  )
